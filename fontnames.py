...
Export OpenType/TrueType name records to editable TOML and apply them
back to a font.
...
def render_toml(source: Path, records: list[dict[str, Any]]) -> str:
    lines = [
        "# Font name table",
        "#",
    ...
    for record in records:
        ...
        lines.append(f"value = {toml_string(record['value'])}")
        lines.append("")
...
def export_font(font_path: Path, output_path: Path | None) -> None:
    ...
    text = render_toml(font_path, records)

    if output_path is None:
        output_path = font_path.with_suffix(".names.toml")

    output_path.write_text(text, encoding="utf-8")
    print(f"Wrote {len(records)} name records to {output_path}")
...
def validate_record(record: dict[str, Any], index: int) -> None:
    ...
    if missing:
        raise ValueError(
            f"names[{index}] is missing: {', '.join(sorted(missing))}"
        )
    ...
    for field in (
        ...
        if not isinstance(record[field], int):
            raise ValueError(
                f"names[{index}].{field} must be an integer"
            )

    if not isinstance(record["value"], str):
        raise ValueError(
            f"names[{index}].value must be a string"
        )
...
def apply_names(
    ...
    if not isinstance(records, list):
        raise ValueError("'names' must be an array of TOML tables")
    ...
    for index, record in enumerate(records):
        ...
        if key in seen:
            raise ValueError(
                f"Duplicate name record at names[{index}]: {key}"
            )
    ...
    try:
        ...
        # The TOML file is authoritative: remove all existing name records.
        name_table.names = []
    ...
    print(f"Wrote rebuilt font to {output_path}")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Inspect and edit OpenType/TrueType font names."
    )
    ...
    show_parser = subparsers.add_parser(
        "show",
        help="Print font name records.",
    )
    show_parser.add_argument("font", type=Path)

    export_parser = subparsers.add_parser(
        "export",
        help="Export name records to editable TOML.",
    )
    export_parser.add_argument("font", type=Path)
    ...
    apply_parser = subparsers.add_parser(
        "apply",
        help="Apply edited TOML to a font.",
    )
    apply_parser.add_argument("font", type=Path)
...
def main() -> int:
    ...
    try:
        if args.command == "show":
            ...
            export_font(args.font, args.output)
